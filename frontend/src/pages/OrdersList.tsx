...
export const OrdersList: React.FC = () => {
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    const matchesMachine =
      filterMachine === 'ALL' || order.processes?.some((p: any) => p.machineId === parseInt(filterMachine, 10));
    ...
    <div>
      ...
      >
        <div>
          ...
          <p style={{ color: 'var(--text-muted)' }}>Gestion integrada de pedidos y especificaciones.</p>
        </div>
      ...
      <div className="card" style={{ marginBottom: '1.5rem', padding: '1.5rem' }}>
        <div className="grid-filter" style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: '1rem' }}>
          ...
          <select className="input" value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)}>
            <option value="ALL">Historico Completo (Incluir Finalizadas)</option>
